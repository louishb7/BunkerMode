...
function setup() {
  ...
  return { prisma, service: new TrackersService(prisma as unknown as PrismaService) };
}
...
describe("Acompanhamentos", () => {
  it("lista todos os acompanhamentos em uma consulta filtrada pelo dono", async () => {
    ...
    expect(prisma.acompanhamentos.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: { objetivo: { usuario_id: 7 } },
  ...
  it("cria apenas dentro de um objetivo do usuário", async () => {
    ...
    prisma.acompanhamentos.create.mockResolvedValue({ id: 11, objetivo_id: 3, ocorrencias: [] });
    await service.create(user, { objetivo_id: 3, titulo: " Não fumar ", descricao: "" });
    expect(prisma.objetivos.findFirst).toHaveBeenCalledWith({ where: { id: 3, usuario_id: 7 }, select: { id: true } });
    expect(prisma.acompanhamentos.create).toHaveBeenCalledWith(expect.objectContaining({
      data: { objetivo_id: 3, titulo: "Não fumar", descricao: null },
    }));
    prisma.objetivos.findFirst.mockResolvedValue(null);
    await expect(service.create(user, { objetivo_id: 4, titulo: "Não fumar" })).rejects.toMatchObject({ status: 404 });
  ...
  it("registra múltiplos eventos reais sem criar tarefas ou dias vazios", async () => {
    ...
    expect(prisma.ocorrencias_acompanhamento.create).toHaveBeenCalledWith({ data: { acompanhamento_id: 11 } });
  ...
  it("remove ocorrência somente do acompanhamento próprio e preserva tarefas ao excluir acompanhamento", async () => {
    const { prisma, service } = setup();
    prisma.acompanhamentos.findFirst.mockResolvedValue({ id: 11 });
    prisma.ocorrencias_acompanhamento.deleteMany.mockResolvedValue({ count: 1 });
    ...
    expect(prisma.acompanhamentos.delete).toHaveBeenCalledWith({ where: { id: 11 } });
    expect(prisma.missoes.delete).not.toHaveBeenCalled();
    prisma.acompanhamentos.findFirst.mockResolvedValue(null);
    await expect(service.recordOccurrence(user, 11)).rejects.toMatchObject({ status: 404 });
  });
});
