...
export default function DaySelector({ onSelectDate, selectedDate, todayDate, weekDays }) {
  return (
    <div className="grid grid-cols-7 gap-0.5 sm:gap-1" aria-label="Dias da semana">
      {weekDays.map((date) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={`min-h-4 text-[10px] font-semibold leading-4 ${today ? "text-accent" : "invisible"}`}
              aria-hidden={!today}
            >
              Hoje
            </span>
