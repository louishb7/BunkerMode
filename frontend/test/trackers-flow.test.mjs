...
test("acompanhamento registra eventos reais, permite remoção e não chama tarefas", async () => {
  ...
  const original = {
    ...
    createTracker: api.createTracker,
    recordTrackerOccurrence: api.recordTrackerOccurrence,
...
test("desvincular série atualiza cache de todas as ocorrências sem remover tarefas", async () => {
  ...
  const original = {
    ...
    materializeTaskRecurrences: api.materializeTaskRecurrences,
  }
  api.materializeTaskRecurrences = async () => ({ ok: true })
  api.listTasks = async () => ({ ok: true, data: tasks })
  api.unlinkTaskFromObjective = async () => {
    tasks = tasks.map((item) =>
      item.recurrence?.series_id === 30
        ? {
            ...item,
            objetivo_id: null,
            recurrence: { ...item.recurrence, termination_policy: "sem_termino" },
          }
        : item
    )
